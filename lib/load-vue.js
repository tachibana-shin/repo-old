		...
		xhr.addEventListener("readystatechange", function () {
			if (xhr.readyState == 4) {
				if ( !trim(xhr.responseText) )
					return reject("load-vue: \"" + url + "\" empty!")
				var x5i609lhm = domify(xhr.responseText), _ = {}, $pwd = dirname(url) || ""
...
.querySelectorAll("script"),
				function (response, el) {
					...
					_.template = (x5i609lhm.querySelector("*") || { outerHTML: "" }).outerHTML

					if ( _.data !== null && typeof _.data == "object" ) {
					  "pwd" in _.data || (_.data.pwd = $pwd)
						!function () {
							var _$data = _.data
							_.data = function () { return _$data }
						}(void 0)
					}
					resolve(extend(object || {}, _))
				})
			}
	 	})
