...
var error = (window.console ? window.console.error || noop : noop).bind(console)
	...
	.defineProperty(local, key, {
    	set: function(newVal) {
			...
			if ( !ofType(getValue(key), sett.type) )
				local[key] = defaultValue(sett.type)
    	},
    	get: function() {
			...
			if ( setting.type !== undefined && setting.type == Function || (isarr(setting.type) && isarr(setting.type).indexOf(Function) > -1) )
				...
				if (getValue(key) === undefined) {
					...
					if ( typeof valDefl == "function" )
						error("Local.js: localStorage don't data is function in default.")
					else local[key] = valDefl
				}
			}
		}()
    return this
...
function clear(e) {
    if (e === undefined) {
       for (var key in settings)
           local[key] = defaultValue(settings[key].type)
       return this
    }
    if ( !isarr(e) )
    	e = [e]
    e.forEach(function(e) {
        ...
        local[e] = defaultValue(settings[e].type)
    })
    return this
}
